// routes/ccAuth.js
// Customer-facing CC authorization e-signature page.
// PCI-safe: only the last 4 digits stored, never full PAN.
  ...
  try {
    ...
    if (!pfR.recordset.length) {
      return res.status(404).send(renderPage('Link Not Found',
        '<div class="error-block"><h2>Authorization Link Not Found</h2>' +
        '<p>This link is invalid or has expired. Please contact Jupiter One USA at (347) 821-7412.</p></div>'));
    }
    ...
    if (existingR.recordset.length) {
      ...
      const alreadyHtml =
        ...
        '<p style="margin-top:20px;">If you need to update, please call (347) 821-7412.</p>' +
    ...
    const form =
      '<div class="auth-header">' +
        '<div class="brand"><h1>JUPITER ONE USA</h1><div class="tagline">AEROSPACE &amp; DEFENSE PARTS SUPPLY</div></div>' +
        '<div class="auth-badge"><div class="badge-label">CREDIT CARD AUTHORIZATION</div><div class="badge-num">For ' + esc(pf.proforma_number) + '</div></div>' +
      '</div>' +
      '<div class="amount-box">' +
        '<div class="amount-label">Amount Authorized</div>' +
        '<div class="amount-value">' + fmtMoney(pf.total) + '</div>' +
        '<div class="amount-sub">Includes 3.5% credit card convenience fee &middot; USD</div>' +
      '</div>' +
      '<form method="POST" action="/cc-auth/' + tokSafe + '" id="ccform" onsubmit="return prepareSubmit()">' +

        '<div class="section">' +
          '<div class="section-num">1</div><div class="section-title">Cardholder Information</div>' +
          '<div class="grid">' +
        ...
        '<div class="section">' +
          '<div class="section-num">2</div><div class="section-title">Card Information</div>' +
          '<div class="pci-notice"><strong>&#128274; Secure:</strong> For your safety, we only store the LAST 4 DIGITS of your card. A Jupiter One USA representative will call you at the number on file to securely collect the full card number for processing.</div>' +
          '<div class="grid">' +
        ...
        '<div class="section">' +
          '<div class="section-num">3</div><div class="section-title">Authorization &amp; Signature</div>' +
          '<div class="auth-text">' +
            'By signing below, I (the cardholder) authorize <strong>Jupiter One USA LLC</strong> to charge the credit card listed above for the amount of <strong>' + fmtMoney(pf.total) + '</strong> in payment of proforma <strong>' + esc(pf.proforma_number) + '</strong> (Order ' + esc(pf.order_number) + ').<br/><br/>' +
          ...
          '<div class="sign-area">' +
            ...
            '<div id="type-pane" style="display:none;">' +
              '<input type="text" id="typed-sig" placeholder="Type your full name as signature" style="font-family:cursive;font-size:32px;"/>' +
            '</div>' +
            '<input type="hidden" name="signature_image" id="sig_image"/>' +
            '<input type="hidden" name="signature_typed" id="sig_typed"/>' +
          '</div>' +
          '<div class="confirm-row">' +
            '<label class="check-label"><input type="checkbox" required/><span>I confirm all information above is accurate and I am authorized to commit this charge.</span></label>' +
          '</div>' +
        '</div>' +
    ...
    res.send(renderPage('Credit Card Authorization', form));
...
router.post('/cc-auth/:token', async (req, res) => {
  try {
    ...
    if (existingR.recordset.length) return res.redirect('/cc-auth/' + tok);
    ...
    await pool.request().input('pid', sql.BigInt, pf.id)
      .query("UPDATE proformas SET status='Authorized', authorized_at=GETDATE(), updated_at=GETDATE() WHERE id=@pid");
    ...
    const okHtml =
      ...
      '</div>' +
      '<p style="margin-top:30px;font-size:13px;color:#7a8a9a;">Reference: ' + esc(pf.proforma_number) + ' &middot; Signed ' + new Date().toLocaleString() + '</p>' +
      '</div>';
    res.send(renderPage('Authorization Submitted', okHtml));
...
function renderPage(title, body) {
  const css =
    ...
    '.submit-btn { display: block; width: 100%; margin: 0; padding: 16px; background: #c8932a; color: #0a1628; font-size: 14px; font-weight: 700; letter-spacing: 0.1em; border: none; cursor: pointer; }' +
  ...
  const script =
    ...
    'function initCanvas() { var c = document.getElementById("sigcanvas"); if (!c) return;' +
    'ctx = c.getContext("2d"); ctx.strokeStyle = "#0a1628"; ctx.lineWidth = 2.2; ctx.lineCap = "round"; ctx.lineJoin = "round";' +
    'function getPos(e) { var r = c.getBoundingClientRect(); var x = (e.touches ? e.touches[0].clientX : e.clientX) - r.left; var y = (e.touches ? e.touches[0].clientY : e.clientY) - r.top; return { x: x * (c.width / r.width), y: y * (c.height / r.height) }; }' +
    ...
    'function switchSign(mode) { var tabs = document.querySelectorAll(".sign-tabs .tab"); tabs.forEach(function(t) { t.classList.remove("active"); });' +
    'if (mode === "draw") { tabs[0].classList.add("active"); document.getElementById("draw-pane").style.display = ""; document.getElementById("type-pane").style.display = "none"; }' +
    'else { tabs[1].classList.add("active"); document.getElementById("draw-pane").style.display = "none"; document.getElementById("type-pane").style.display = ""; } }' +
    'function prepareSubmit() { var typed = document.getElementById("typed-sig"); if (typed && typed.value.trim().length > 0) { document.getElementById("sig_typed").value = typed.value.trim(); return true; }' +
    'if (!hasDrawn) { alert("Please sign by drawing or typing your name."); return false; }' +
    'var c = document.getElementById("sigcanvas"); document.getElementById("sig_image").value = c.toDataURL("image/png"); return true; }' +
  ...
  return '<!doctype html><html><head><meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1"/>' +
    '<title>' + esc(title) + ' - Jupiter One USA</title><style>' + css + '</style></head>' +
