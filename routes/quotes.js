  ...
  try {
    ...
    try {
      ...
      const orderResult = await pool.request()
        ...
        .query(`
          INSERT INTO orders (quote_id, rfq_id, customer_id, order_number, subtotal, total_amount, payment_terms, status)
      ...
      // Log order status
      await pool.request().input('orderId', sql.BigInt, order.id)
        .query(`INSERT INTO order_status_log (order_id, new_status, note) VALUES (@orderId, 'Confirmed', 'Order auto-created from accepted quote')`);
      // Send confirmation email
      const custResult = await pool.request().input('cid', sql.BigInt, req.customerId)
      ...
      if (custResult.recordset.length) {
        ...
        sendOrderConfirmation({ customer: custResult.recordset[0], order: { ...order, total_amount: q.total_amount, payment_terms: q.payment_terms } }).catch(console.error);
      }
      console.log('Order auto-created:', orderNumber);
    } catch(orderErr) { console.error('Auto-order creation error:', orderErr.message); }
