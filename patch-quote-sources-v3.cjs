// patch-quote-sources-v3.cjs
// v3: Scope the mapper search to within the GET /quotes/:id handler.
// There are multiple lineRows mappers in the file; we want the one for quote detail.
...
console.log('Quote handler starts at L' + (handlerStart+1));

// Find the lines query within that handler (within ~50 lines after start)
...
if (linesQueryLineIdx < 0) {
  console.error('! quote_lines query not found in handler');
  process.exit(1);
}
console.log('Lines query at L' + (linesQueryLineIdx+1));

// Find the lineRows mapper WITHIN this handler (after the query, before end of route)
let lineRowsIdx = -1;
for (let i = linesQueryLineIdx + 1; i < Math.min(linesQueryLineIdx + 30, lines.length); i++) {
  if (lines[i].includes('const lineRows = lines.recordset.map')) { lineRowsIdx = i; break; }
}
if (lineRowsIdx < 0) {
  console.error('! lineRows mapper not found after L' + (linesQueryLineIdx+1));
  console.error('Lines after query:');
  for (let i = linesQueryLineIdx; i < Math.min(linesQueryLineIdx + 30, lines.length); i++) {
    console.error('  L' + (i+1) + ': ' + lines[i].substring(0, 150));
  }
  process.exit(1);
}
console.log('Mapper at L' + (lineRowsIdx+1));

// Find the mapper close — look for </tr>`).join after lineRowsIdx, within 30 lines
let mapperCloseIdx = -1;
for (let i = lineRowsIdx; i < Math.min(lineRowsIdx + 30, lines.length); i++) {
  if (lines[i].includes("</tr>`).join(")) {
    mapperCloseIdx = i; break;
  }
}
if (mapperCloseIdx < 0) {
  console.error('! mapper close line not found');
  for (let i = lineRowsIdx; i < Math.min(lineRowsIdx + 30, lines.length); i++) {
    console.error('  L' + (i+1) + ': ' + lines[i].substring(0, 200));
  }
  process.exit(1);
}
console.log('Mapper close at L' + (mapperCloseIdx+1));
console.log('Close line content: [' + lines[mapperCloseIdx] + ']');

// === Insert source-load block right after the lines query line ===
const sourceLoad = [
  '',
...
// Indices have shifted by the # of lines we just inserted
const insertedLineCount = sourceLoad.split('\n').length;
lineRowsIdx += insertedLineCount;
mapperCloseIdx += insertedLineCount;

// === Insert renderer helper BEFORE the lineRows declaration ===
const helperBlock = [
...
const helperLineCount = helperBlock.split('\n').length;
mapperCloseIdx += helperLineCount;

// === Now replace `</tr>`).join('')` with `</tr>${_renderSourceSubRow(_sourcesByLine[l.id])}`).join('')` ===
// The close line currently has `</tr>\`).join(...)`. We just need to insert ${...} before the backtick.
const before = lines[mapperCloseIdx];
const replaced = before.replace(/<\/tr>`\)\.join\(/, "</tr>${_renderSourceSubRow(_sourcesByLine[l.id])}`).join(");
if (replaced === before) {
  console.error('! could not replace mapper close. Content:');
  console.error('  [' + before + ']');
  process.exit(1);
}
lines[mapperCloseIdx] = replaced;
console.log('Replaced close: [' + replaced + ']');

// Marker at top so we don't re-run
