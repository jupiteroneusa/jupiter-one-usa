...
.po-box .number { font-size: 18pt; font-weight: 700; color: #c8932a; margin-top: 4px; font-family: 'Courier New', monospace; letter-spacing: 1px; }
...
table.lines tbody .pn { font-family: 'Courier New', monospace; color: #c8932a; font-weight: 700; font-size: 10pt; }
...
.totals tr.grand td { font-size: 13pt; color: #c8932a; font-weight: 700; border-top: 2px solid #c8932a; padding-top: 10px; }
...
<div class="header">
  <div class="brand-block">
    ...
    <div class="addr">400 N Tampa St, Suite 1550<br/>Tampa, FL 33602<br/>(347) 821-7412 &middot; DTorchia@JupiterOneUSA.com</div>
  </div>
  <div class="po-box">
...
<div class="footer">Jupiter One USA &middot; CAGE Code on request &middot; This PO is electronically issued and valid without signature</div>
  ...
  // back-to-back on small Azure plans)
  let browser, lastErr;
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      lastErr = err;
      if (err.code === 'ETXTBSY' || /ETXTBSY|spawn|EBUSY/i.test(err.message || '')) {
        console.warn('[poPdf] Launch attempt ' + attempt + '/3 failed: ' + err.message + ' — retrying in 1.5s');
