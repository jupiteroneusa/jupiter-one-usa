// patch-quote-sources-v5.cjs
// v4 had multi-line single-quoted string literals which broke when newlines
// landed inside them. v5 uses + concatenation entirely on separate
// statements, with each string self-contained on its own logical line.
...
if (matches !== 1) {
  console.error('! lines query is not unique. Count: ' + matches);
  process.exit(1);
}

// Build the injection. Critical: each JS string literal is fully on ONE line.
// No multi-line strings. Use concatenation across separate statements.
const inject = [
  linesQueryStr,
