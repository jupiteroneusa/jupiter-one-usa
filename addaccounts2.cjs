...
// 1. Add Accounts to sidebar - find the rfqs link end and insert after
const rfqsLink = 'href="/admin/rfqs"';
const idx = a.indexOf(rfqsLink);
if (idx > -1) {
  const endIdx = a.indexOf('</a>', idx) + 4;
  a = a.slice(0, endIdx) + '\n    <a href="/admin/accounts" class="${active===\'accounts\'?\'active\':\'\'}">\uD83C\uDFE2 Accounts</a>' + a.slice(endIdx);
  console.log('Sidebar: FIXED');
} else console.log('Sidebar: NOT FOUND');

// 2. Add accounts routes before customers list
...
const accountsCode = `  // Accounts
  router.get('/accounts', async (req, res) => {
    ...
    try {
        ...
        if (type === 'ref') {
          ...
          rows = result.recordset.map(function(r) {
            return '<tr>' +
              ...
              '<td style="color:#7a8a9a;">' + (r.customer_name||'') + '</td>' +
      ...
      const tableHtml = search ? ('<div class="card"><div class="card-header">Results for &quot;' + search + '&quot;</div><div style="overflow-x:auto;"><table><thead><tr>' + headers + '</tr></thead><tbody>' + rows + '</tbody></table></div></div>') : '<div style="text-align:center;padding:60px;color:#7a8a9a;"><div style="font-size:2rem;margin-bottom:12px;">&#128194;</div><div>Search by company or customer reference</div></div>';
      res.send(page('Accounts','accounts', SORT_SCRIPT +
        ...
        '<select name="type" style="width:150px;"><option value="company"' + (!isRef?' selected':'') + '>Company</option><option value="ref"' + (isRef?' selected':'') + '>Customer Ref</option></select></div>' +
  ...
  router.get('/accounts/ref/:ref', async (req, res) => {
    ...
    try {
      ...
      res.send(page('Ref: ' + ref,'accounts',
        ...
        '<div class="page-sub">All activity for this reference</div>' +
  ...
  router.get('/accounts/company/:company', async (req, res) => {
    ...
    try {
      const pool = await getPool();
      const contacts = await pool.request().input('co', sql.NVarChar, company).query(\`
        SELECT id, first_name+' '+last_name AS name, email, phone, status, created_at
        FROM customers WHERE company=@co ORDER BY last_name
      \`);
...
if (a.includes(insertBefore)) {
  ...
  console.log('Accounts routes: NOT FOUND');
}
