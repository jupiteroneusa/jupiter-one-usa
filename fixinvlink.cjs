// fixinvlink.cjs
// Adds clickable invoice numbers (in order payment tab + invoice list)
// and creates the /admin/invoices/:id detail page route.
...
console.log('Jupiter One - Invoice link + detail page patch');
console.log('-----------------------------------------------');
...
patch(
  ...
  `'<tr><td class="mono" style="color:#c8932a;">'+inv.invoice_number+'</td>'`,
  `'<tr><td class="mono"><a href="/admin/invoices/'+inv.id+'" style="color:#c8932a;text-decoration:none;">'+inv.invoice_number+'</a></td>'`,
  'orderRoutes.js - clickable invoice # on payment tab'
);

// =========================================================================
// PATCH 2: admin/index.js - link invoice # on invoice list
// =========================================================================
patch(
  'admin/index.js',
  ...
  'index.js - clickable invoice # on invoice list'
);

// =========================================================================
// PATCH 3: admin/index.js - add /admin/invoices/:id detail route
// =========================================================================
const indexFile = 'admin/index.js';
let idx = fs.readFileSync(indexFile, 'utf8');

const detailRouteMarker = "router.get('/invoices/:id'";
if (idx.includes(detailRouteMarker)) {
  console.log('  - Already patched: invoice detail route exists');
} else {
  // Insert the new route immediately before the Suppliers section.
  const anchor = "  // Suppliers\n  router.get('/suppliers'";

  if (!idx.includes(anchor)) {
    console.error('  ! Could not find Suppliers anchor to insert invoice detail route');
  } else {
    const detailRoute = `  // Invoice Detail
...
`;
    ...
    idx = idx.replace(anchor, detailRoute + anchor);
...
console.log('-----------------------------------------------');
console.log('Done. Restart the server and click an invoice #.');
