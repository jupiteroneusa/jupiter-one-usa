// patch-quote-sources-v8.cjs
// THE BUG: String.prototype.replace() with a STRING second arg treats $&, $1, $', etc
// as special. My injection contained '$' (literal dollar quote) which got interpreted
// as "the substring after the match" and corrupted the output.
// FIX: pass replacement as a FUNCTION instead. Function replacements don't apply $-substitution.
...
// FIX: use function replacement to avoid $-substitution
s = s.replace(linesQueryStr, function() { return inject; });

const unitCostMarker = 'parseFloat(l.unit_cost||0).toFixed(2)';
const ucIdx = s.indexOf(unitCostMarker);
if (ucIdx < 0) { console.error('! unit_cost marker not found'); process.exit(1); }

const closeNeedle = "</tr>`).join('');";
const closeIdx = s.indexOf(closeNeedle, ucIdx);
if (closeIdx < 0) { console.error('! mapper close not found'); process.exit(1); }

// Manual replacement here too (substring slicing, no replace())
...
try {
  ...
  console.log('+ Shows supplier / qty / unit cost / lead time / certs');
