...
function renderCocTab(o, oLines, certs) {
  function esc(t){ return String(t==null?'':t).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }
  var byLine = {};
  (certs||[]).forEach(function(c){ if(!byLine[c.order_line_id]) byLine[c.order_line_id]=[]; byLine[c.order_line_id].push(c); });
  var h = '<div style="padding:4px 0;">';
  h += '<div style="font-size:.72rem;letter-spacing:.12em;text-transform:uppercase;color:#c8932a;font-weight:700;margin-bottom:6px;">Certificates of Conformance</div>';
  h += '<div style="font-size:.8rem;color:#cfd5dc;margin-bottom:16px;">ATA Spec 106 cert per line. Add another for a split batch/lot.</div>';
  (oLines.recordset||[]).forEach(function(l){
    var partLabel = esc(l.part_number || l.nsn || ('Line ' + l.line_number));
    h += '<div style="background:#0e1828;border:1px solid #1e2d42;border-radius:6px;padding:14px 16px;margin-bottom:14px;">';
    h += '<div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-bottom:8px;">';
    h += '<div style="font-weight:600;color:#eef1f5;font-size:.9rem;">Line ' + l.line_number + ' &mdash; ' + partLabel + ' <span style="color:#7a8a9a;font-weight:400;">(qty ' + l.quantity_ordered + ')</span></div>';
    h += '<form method="POST" action="/admin/orders/' + o.id + '/lines/' + l.id + '/coc/generate" style="margin:0;"><button type="submit" class="btn btn-gold btn-sm" style="font-size:.75rem;">+ Generate CoC</button></form>';
    h += '</div>';
    var rows = byLine[l.id] || [];
    if (!rows.length) {
      h += '<div style="color:#7a8a9a;font-size:.8rem;padding:6px 0;">No certificate yet.</div>';
    } else {
      h += '<table style="width:100%;border-collapse:collapse;"><thead><tr style="border-bottom:1px solid #1e2d42;">';
...
export function mountOrderRoutes(router, requireAuth, page) {
  ...
  router.get('/orders/:id', async (req, res) => {
    ...
    try {
      ...
      html += tabLink('overview','&#128203; Overview')+tabLink('lines','&#128230; Lines')+tabLink('shipping','&#128666; Shipping') + tabLink('proforma','&#129534; Proforma')+tabLink('payment','&#128179; Payment')+tabLink('documents','&#128206; Documents')+tabLink('coc','&#128203; CoC'); /* ORDER_DOCS_TAB_v1 COC_TAB_v1 */
      ...
      } else if (activeTab === 'coc') { /* COC_TAB_v1 */
        const _cocR = await pool.request().input('oidCoc', sql.BigInt, req.params.id)
          .query('SELECT c.id, c.coc_number, c.status, c.quantity, c.serial_batch_number, c.order_line_id FROM coc_certificates c WHERE c.order_id=@oidCoc ORDER BY c.order_line_id, c.id');
        html += renderCocTab(o, oLines, _cocR.recordset);
      }
      html += '</div></div>';
  ...
  /* COC_ROUTES_v1: generate / edit / save Certificate of Conformance (ATA Spec 106) */
  router.post('/orders/:id/lines/:lineId/coc/generate', async (req, res) => {
    if (!requireAuth(req, res)) return;
    try {
      const pool = await getPool();
      const orderId = parseInt(req.params.id);
      const lineId = parseInt(req.params.lineId);
      const ordR = await pool.request().input('oid', sql.BigInt, orderId)
        .query('SELECT o.*, c.company AS cust_company FROM orders o LEFT JOIN customers c ON c.id=o.customer_id WHERE o.id=@oid');
      const o = ordR.recordset[0];
      if (!o) return res.redirect('/admin/orders?error=Order+not+found');
      const lnR = await pool.request().input('lid', sql.BigInt, lineId)
        .query('SELECT * FROM order_lines WHERE id=@lid');
      const ln = lnR.recordset[0];
      if (!ln) return res.redirect('/admin/orders/' + orderId + '?tab=coc&error=Line+not+found');
      /* COC_PER_RECEIVED_SOURCE_v1: one CoC per RECEIVED source (dedupe by order_line_source_id). MARKER COC_PER_RECEIVED_SOURCE_v1 */
      const srcR = await pool.request().input('lid2', sql.BigInt, lineId)
        .query('SELECT ols.id AS source_id, ols.allocated_qty, ols.received_qty, ols.supplier_id, ols.supplier_po_line_id, s.company_name AS supplier_name, pl.received_quantity AS po_received, pl.quantity AS po_qty, pl.part_number AS po_part, pl.item_name AS po_item, pl.condition_code AS po_cond, sp.po_number FROM order_line_sources ols LEFT JOIN suppliers s ON s.id = ols.supplier_id LEFT JOIN supplier_po_lines pl ON pl.id = ols.supplier_po_line_id LEFT JOIN supplier_pos sp ON sp.id = pl.supplier_po_id WHERE ols.order_line_id=@lid2 ORDER BY ols.sort_order, ols.id');
      const allSources = srcR.recordset || [];
      /* received = its PO line has received_quantity > 0 (fallback to source.received_qty if directly tracked) */
      const receivedSources = allSources.filter(function(r){ return (r.po_received && r.po_received > 0) || (r.received_qty && r.received_qty > 0); });
      if (!receivedSources.length) {
        return res.redirect('/admin/orders/' + orderId + '?tab=coc&error=' + encodeURIComponent('No received sources on this line yet. Receive the supplier PO first.'));
      }
      /* dedupe: skip sources that already have a CoC */
      const existCocR = await pool.request().input('lid3', sql.BigInt, lineId)
        .query('SELECT order_line_source_id FROM coc_certificates WHERE order_line_id=@lid3 AND order_line_source_id IS NOT NULL');
      const haveCoc = new Set((existCocR.recordset || []).map(function(r){ return String(r.order_line_source_id); }));
      const toMake = receivedSources.filter(function(r){ return !haveCoc.has(String(r.source_id)); });
      const statusMap = { NE:'New', NS:'New Surplus', OH:'Overhauled', RP:'Repaired', SV:'Serviceable', AR:'As Removed', SC:'Scrap' };
      let firstCocId = null;
      if (!toMake.length) {
        /* all received sources already have certs; go to the first existing cert for this line */
        const firstR = await pool.request().input('lid4', sql.BigInt, lineId)
          .query('SELECT TOP 1 id FROM coc_certificates WHERE order_line_id=@lid4 ORDER BY id');
        if (firstR.recordset && firstR.recordset[0]) firstCocId = firstR.recordset[0].id;
      }
      for (let i = 0; i < toMake.length; i++) {
        const src = toMake[i];
        const numR = await pool.request().query("SELECT MAX(TRY_CONVERT(int, REPLACE(coc_number,'COFC-',''))) AS mx FROM coc_certificates WHERE coc_number LIKE 'COFC-%'");
        const nextNum = ((numR.recordset[0] && numR.recordset[0].mx) || 0) + 1;
        const cocNumber = 'COFC-' + String(nextNum).padStart(5, '0');
        const qty = (src.po_received && src.po_received > 0) ? src.po_received : (src.received_qty || src.allocated_qty || null);
        const serialBatch = ln.lot_number || (ln.serial_numbers || null);
        const condCode = src.po_cond || ln.condition_code || '';
        const statusCode = statusMap[(condCode || '').toUpperCase()] || (condCode || null);
        const partPn = src.po_part || ln.part_number || null;
        const descr = src.po_item || ln.item_name || null;
        const ins = await pool.request()
          .input('order_id', sql.BigInt, orderId)
          .input('order_line_id', sql.BigInt, lineId)
          ...
          .input('eligibility', sql.NVarChar(150), null)
          ...
          .query('INSERT INTO coc_certificates (order_id, order_line_id, order_line_source_id, coc_number, reference_number, seller_contract_number, buyer_contract_number, item_number, description, manufacturer_pn, eligibility, quantity, serial_batch_number, status_code, remarks, obtained_from, last_certified_agency, status, created_at, updated_at) OUTPUT INSERTED.id VALUES (@order_id, @order_line_id, @order_line_source_id, @coc_number, @reference_number, @seller_contract_number, @buyer_contract_number, @item_number, @description, @manufacturer_pn, @eligibility, @quantity, @serial_batch_number, @status_code, @remarks, @obtained_from, @last_certified_agency, @st, GETDATE(), GETDATE())');
        if (firstCocId === null) firstCocId = ins.recordset[0].id;
      }
      if (!firstCocId) { return res.redirect('/admin/orders/' + orderId + '?tab=coc'); }
            return res.redirect('/admin/orders/' + orderId + '/coc/' + firstCocId + '/edit');
    } catch (err) {
      console.error('CoC generate error:', err);
      res.redirect('/admin/orders/' + req.params.id + '?tab=coc&error=' + encodeURIComponent(err.message));
    }
  });

  router.get('/orders/:id/coc/:cocId/edit', async (req, res) => {
    if (!requireAuth(req, res)) return;
    try {
      const pool = await getPool();
      const orderId = parseInt(req.params.id);
      const cocId = parseInt(req.params.cocId);
      const cR = await pool.request().input('cid', sql.BigInt, cocId)
        .query('SELECT * FROM coc_certificates WHERE id=@cid');
      const c = cR.recordset[0];
      if (!c) return res.redirect('/admin/orders/' + orderId + '?tab=coc&error=CoC+not+found');
      const AMP = String.fromCharCode(38);
      const LT = String.fromCharCode(60);
      const QUOT = String.fromCharCode(34);
      function v(x){ if(x==null) return ''; return String(x).split(AMP).join('&amp;').split(LT).join('&lt;').split(QUOT).join('&quot;'); }
      function dv(x){ return x==null ? '' : (new Date(x)).toISOString().slice(0,10); }
      function inp(name, val){ return '<input type=' + QUOT + 'text' + QUOT + ' name=' + QUOT + name + QUOT + ' value=' + QUOT + v(val) + QUOT + ' style=' + QUOT + 'width:100%;background:#0a1628;border:1px solid #1e2d42;color:#eef1f5;padding:6px 8px;font-size:.82rem;' + QUOT + '/>'; }
      function lbl(t){ return '<div style=' + QUOT + 'font-size:.62rem;color:#7a8a9a;text-transform:uppercase;letter-spacing:.08em;margin-bottom:3px;' + QUOT + '>' + t + '</div>'; }
      function fld(label, name, val){ return '<div>' + lbl(label) + inp(name, val) + '</div>'; }
      function datefld(label, name, val){ return '<div>' + lbl(label) + '<input type=' + QUOT + 'date' + QUOT + ' name=' + QUOT + name + QUOT + ' value=' + QUOT + dv(val) + QUOT + ' style=' + QUOT + 'width:100%;background:#0a1628;border:1px solid #1e2d42;color:#eef1f5;padding:6px 8px;font-size:.82rem;' + QUOT + '/></div>'; }
      var h = '<div style=' + QUOT + 'max-width:900px;margin:0 auto;' + QUOT + '>';
      h += '<div style=' + QUOT + 'display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;' + QUOT + '>';
      h += '<h1 style=' + QUOT + 'margin:0;' + QUOT + '>' + v(c.coc_number) + ' <span style=' + QUOT + 'font-size:.6em;color:#7a8a9a;' + QUOT + '>' + v(c.status) + '</span></h1>';
      h += '<a href=' + QUOT + '/admin/orders/' + orderId + '?tab=coc' + QUOT + ' class=' + QUOT + 'btn btn-outline btn-sm' + QUOT + '>&#8592; Back to CoC</a></div>';
      h += '<div style=' + QUOT + 'font-size:.8rem;color:#7a8a9a;margin-bottom:18px;' + QUOT + '>ATA Spec 106 &mdash; Part or Material Certification Form</div>';
      h += '<form method=' + QUOT + 'POST' + QUOT + ' action=' + QUOT + '/admin/orders/' + orderId + '/coc/' + cocId + '/save' + QUOT + '>';
      h += '<div class=' + QUOT + 'card' + QUOT + '><div class=' + QUOT + 'card-body' + QUOT + '>';
      h += '<div style=' + QUOT + 'display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:14px;' + QUOT + '>';
      h += fld('3. Reference Number','reference_number',c.reference_number);
      h += fld('5a. Seller Contract #','seller_contract_number',c.seller_contract_number);
      h += fld('5b. Buyer Contract #','buyer_contract_number',c.buyer_contract_number);
      h += '</div>';
      h += '<div style=' + QUOT + 'display:grid;grid-template-columns:0.5fr 2fr 1.2fr;gap:12px;margin-bottom:14px;' + QUOT + '>';
      h += fld('6. Item','item_number',c.item_number);
      h += fld('7. Description','description',c.description);
      h += fld('8. Manufacturer & PN','manufacturer_pn',c.manufacturer_pn);
      h += '</div>';
      h += '<div style=' + QUOT + 'display:grid;grid-template-columns:1.2fr 0.5fr 1fr 0.8fr;gap:12px;margin-bottom:14px;' + QUOT + '>';
      h += fld('9. Eligibility','eligibility',c.eligibility);
      h += fld('10. Qty','quantity',c.quantity);
      h += fld('11. Serial/Batch #','serial_batch_number',c.serial_batch_number);
      h += fld('12. Status','status_code',c.status_code);
      h += '</div>';
      h += '<div style=' + QUOT + 'margin-bottom:14px;' + QUOT + '>' + lbl('13a. Remarks') + '<textarea name=' + QUOT + 'remarks' + QUOT + ' rows=' + QUOT + '2' + QUOT + ' style=' + QUOT + 'width:100%;background:#0a1628;border:1px solid #1e2d42;color:#eef1f5;padding:6px 8px;font-size:.82rem;' + QUOT + '>' + v(c.remarks) + '</textarea></div>';
      h += '<div style=' + QUOT + 'display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px;' + QUOT + '>';
      h += fld('13b. Obtained From','obtained_from',c.obtained_from);
      h += fld('13c. Last Certified Agency','last_certified_agency',c.last_certified_agency);
      h += '</div>';
      h += '<div style=' + QUOT + 'border-top:1px solid #1e2d42;padding-top:14px;margin-top:6px;' + QUOT + '>' + '<div style=' + QUOT + 'font-size:.7rem;color:#c8932a;text-transform:uppercase;letter-spacing:.1em;font-weight:700;margin-bottom:10px;' + QUOT + '>Block 14 &mdash; New Parts / Material Verification</div>';
      h += '<div style=' + QUOT + 'display:grid;grid-template-columns:1fr 1fr 0.8fr;gap:12px;' + QUOT + '>';
      h += fld('Signature','block14_signature',c.block14_signature);
      h += fld('Name','block14_name',c.block14_name);
      h += datefld('Date','block14_date',c.block14_date);
      h += '</div></div>';
      h += '<div style=' + QUOT + 'border-top:1px solid #1e2d42;padding-top:14px;margin-top:14px;' + QUOT + '>' + '<div style=' + QUOT + 'font-size:.7rem;color:#c8932a;text-transform:uppercase;letter-spacing:.1em;font-weight:700;margin-bottom:10px;' + QUOT + '>Block 18 &mdash; New/Used/Repaired/Overhauled/Surplus Verification</div>';
      h += '<div style=' + QUOT + 'display:grid;grid-template-columns:1fr 1fr 0.8fr;gap:12px;' + QUOT + '>';
      h += fld('Signature','block18_signature',c.block18_signature);
      h += fld('Name','block18_name',c.block18_name);
      h += datefld('Date','block18_date',c.block18_date);
      h += '</div></div>';
      h += '<div style=' + QUOT + 'display:flex;justify-content:flex-end;gap:10px;margin-top:18px;' + QUOT + '>';
      h += '<button type=' + QUOT + 'submit' + QUOT + ' name=' + QUOT + '_status' + QUOT + ' value=' + QUOT + 'Draft' + QUOT + ' class=' + QUOT + 'btn btn-outline' + QUOT + '>Save Draft</button>';
      h += '<button type=' + QUOT + 'submit' + QUOT + ' name=' + QUOT + '_status' + QUOT + ' value=' + QUOT + 'Issued' + QUOT + ' class=' + QUOT + 'btn btn-gold' + QUOT + '>Save &amp; Mark Issued</button>';
      h += '</div>';
      h += '</div></div></form></div>';
      res.send(page('CoC ' + v(c.coc_number), 'orders', h)); /* COC_PAGEFIX_v1 */
    } catch (err) {
  ...
  router.post('/orders/:id/coc/:cocId/save', async (req, res) => {
    if (!requireAuth(req, res)) return;
    try {
      const pool = await getPool();
      const orderId = parseInt(req.params.id);
      const cocId = parseInt(req.params.cocId);
      const b = req.body || {};
      const newStatus = (b._status === 'Issued') ? 'Issued' : 'Draft';
      const qtyVal = (b.quantity !== undefined && b.quantity !== '') ? parseInt(b.quantity) : null;
      const itemVal = (b.item_number !== undefined && b.item_number !== '') ? parseInt(b.item_number) : null;
      await pool.request()
        .input('cid', sql.BigInt, cocId)
        .input('reference_number', sql.NVarChar(50), b.reference_number || null)
        .input('seller_contract_number', sql.NVarChar(50), b.seller_contract_number || null)
        .input('buyer_contract_number', sql.NVarChar(50), b.buyer_contract_number || null)
        .input('item_number', sql.Int, itemVal)
        .input('description', sql.NVarChar(255), b.description || null)
        .input('manufacturer_pn', sql.NVarChar(150), b.manufacturer_pn || null)
        .input('eligibility', sql.NVarChar(150), b.eligibility || null)
        .input('quantity', sql.Int, qtyVal)
        .input('serial_batch_number', sql.NVarChar(100), b.serial_batch_number || null)
        .input('status_code', sql.NVarChar(20), b.status_code || null)
        .input('remarks', sql.NVarChar(sql.MAX), b.remarks || null)
        .input('obtained_from', sql.NVarChar(255), b.obtained_from || null)
        .input('last_certified_agency', sql.NVarChar(255), b.last_certified_agency || null)
        .input('block14_signature', sql.NVarChar(150), b.block14_signature || null)
        .input('block14_name', sql.NVarChar(150), b.block14_name || null)
        .input('block14_date', sql.Date, b.block14_date || null)
        .input('block18_signature', sql.NVarChar(150), b.block18_signature || null)
        .input('block18_name', sql.NVarChar(150), b.block18_name || null)
        .input('block18_date', sql.Date, b.block18_date || null)
        .input('status', sql.NVarChar(30), newStatus)
        .query('UPDATE coc_certificates SET reference_number=@reference_number, seller_contract_number=@seller_contract_number, buyer_contract_number=@buyer_contract_number, item_number=@item_number, description=@description, manufacturer_pn=@manufacturer_pn, eligibility=@eligibility, quantity=@quantity, serial_batch_number=@serial_batch_number, status_code=@status_code, remarks=@remarks, obtained_from=@obtained_from, last_certified_agency=@last_certified_agency, block14_signature=@block14_signature, block14_name=@block14_name, block14_date=@block14_date, block18_signature=@block18_signature, block18_name=@block18_name, block18_date=@block18_date, status=@status, updated_at=GETDATE() WHERE id=@cid');
      res.redirect('/admin/orders/' + orderId + '?tab=coc&saved=1');
    } catch (err) {
      console.error('CoC save error:', err);
      res.redirect('/admin/orders/' + req.params.id + '/coc/' + req.params.cocId + '/edit?error=' + encodeURIComponent(err.message));
    }
  });
  ...
  router.post('/orders/:id/generate-invoice', async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        const mailOpts = { from: '"Jupiter One USA" <nicolle@jupiteroneusa.com>', to: o.email, cc: adminCopy.toLowerCase() !== String(o.email).toLowerCase() ? adminCopy : undefined, bcc: 'nicolle@jupiteroneusa.com', subject: (isPaidInFull ? 'PAID: Invoice ' : 'Invoice ')+invoiceNumber+' — Jupiter One USA', html: emailHtml };
  ...
  router.post('/orders/:id/send-proforma', async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        const mailOpts = {
          from: '"Jupiter One USA" <' + (process.env.ADMIN_EMAIL || 'nicolle@jupiteroneusa.com') + '>',
          to: _primaryTo,
          cc: _ccArr.length ? _ccArr : undefined,
          bcc: process.env.ADMIN_EMAIL || 'nicolle@jupiteroneusa.com',
