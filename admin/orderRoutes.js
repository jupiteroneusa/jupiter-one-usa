...
export function mountOrderRoutes(router, requireAuth, page) {
  ...
  router.get('/orders/:id', async (req, res) => {
    ...
    try {
      ...
      if (activeTab === 'overview') {
        ...
        /* ADD_ORDER_LINE_v1: add-a-line-to-existing-order form */
        html += '<div style="margin-bottom:16px;">';
        html += '<button type="button" onclick="var d=document.getElementById(\u0027addLineForm\u0027);d.style.display=d.style.display===\u0027none\u0027?\u0027block\u0027:\u0027none\u0027;" class="btn btn-outline btn-sm" style="border-color:#c8932a;color:#c8932a;">+ Add Line to Order</button>';
        html += '<div id="addLineForm" style="display:none;margin-top:12px;background:#0e1828;border:1px solid #1e2d42;border-radius:6px;padding:16px;">';
        ...
        html += '</form>';
        html += '<div style="font-size:.72rem;color:#7a8a9a;margin-top:10px;">Adds a new line to this order and updates the order total. Source and ship it like any other line.</div>';
        html += '</div></div>';
  ...
  router.post('/orders/:id/lines/add', async (req, res) => {
    ...
    try {
      ...
      // Recompute order subtotal + total (subtotal = sum of line_totals; keep tax + shipping)
      ...
      await pool.request().input('oid', sql.BigInt, orderId).input('n', sql.NVarChar(500), 'Line ' + nextLn + ' added' + (b.part_number ? ' (' + b.part_number + ')' : '') + ' - qty ' + qty + ' @ 
    if (!requireAuth(req, res)) return;
    try {
      const pool = await getPool();
      const b = req.body;
