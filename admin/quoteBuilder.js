...
export function mountQuoteBuilder(router, requireAuth, page) {
  ...
  router.post('/rfqs/:id/quote', async (req, res) => {
    ...
    try {
      ...
      /* QUOTE_DRAFTS_v2: clean up draft on successful save */
try {
  const pool2 = await getPool();
  await pool2.request().input('rfqIdDel', sql.BigInt, req.params.id).query('DELETE FROM quote_drafts WHERE rfq_id=@rfqIdDel');
