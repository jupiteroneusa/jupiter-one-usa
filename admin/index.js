...
export async function buildAdminRouter() {
  ...
  router.get('/orders/:id', async (req, res) => {
    ...
    try {
      ...
      const shipRows = shipments.recordset.map(s => `<tr>
      ...
      if (activeTab === 'overview') {
        tabContent = `
          <div class="detail-grid" style="margin-bottom:20px;">
            ...
            <div class="detail-item"><div class="detail-label">Shipping</div><div class="detail-value">${o.shipping_cost?'$'+parseFloat(o.shipping_cost).toFixed(2):'<span style="color:#e05050;">Not set</span>'}</div></div>
        ...
        tabContent = `
          ...
          <div style="padding:16px;text-align:right;border-top:1px solid #1e2d42;">
            ...
            ${o.shipping_cost?'<span style="color:#7a8a9a;margin-right:16px;">Shipping: <strong>$'+parseFloat(o.shipping_cost).toFixed(2)+'</strong></span>':''}
        ...
        tabContent = `
          <div class="card"><div class="card-header">Mark as Paid</div><div class="card-body">
            <div style="margin-bottom:16px;font-size:.88rem;color:#7a8a9a;">
              ...
              ${o.shipping_cost?' + Shipping: <strong style="color:#eef1f5;">$'+parseFloat(o.shipping_cost).toFixed(2)+'</strong>':'<span style="color:#e05050;"> (shipping not added)</span>'}
            ...
            ${o.status==='Paid'?'<div class="alert alert-success">&#10004; Order is marked Paid.</div>':''}
            <form method="POST" action="/admin/orders/${o.id}/mark-paid" style="display:flex;gap:12px;flex-wrap:wrap;align-items:flex-end;" onsubmit="return confirm('Mark as Paid?')">
              <div><div style="font-size:.7rem;color:#7a8a9a;margin-bottom:4px;">Payment Method</div>
                <select name="payment_method"><option>Wire Transfer</option><option>Credit Card</option><option>Check</option><option>Other</option></select>
              </div>
      ...
      res.send(page(`Order ${o.order_number}`,'orders',`
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;flex-wrap:wrap;gap:8px;">
          ...
          <div class="page-sub" style="margin-bottom:0;">${o.customer_name} &middot; ${o.company||''}</div></div>
          <a href="/admin/orders" class="btn btn-outline btn-sm">&#8592; Back</a>
        </div>
        <div style="border-bottom:1px solid #1e2d42;margin-bottom:24px;overflow-x:auto;white-space:nowrap;">
          ${tabLink('overview','&#128203; Overview')}
  ...
  router.post('/orders/:id/tracking', async (req, res) => {
    ...
    try {
      ...
      await pool.request().input('id', sql.BigInt, req.params.id)
        .query("INSERT INTO order_status_log (order_id,new_status,note) VALUES (@id,'Shipped','Shipment added')");
      res.redirect('/admin/orders/'+req.params.id+'?tab=shipping&saved=1');
    } catch(err) { res.redirect('/admin/orders/'+req.params.id+'?error='+encodeURIComponent(err.message)); }
  });
