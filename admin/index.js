  ...
  router.get('/quotes/:id/edit', async (req, res) => {
    ...
    try {
      ...
      linesR.recordset.forEach(function(l, idx) {
        ...
        /* QUOTE_COND_DROPDOWN_v1: condition is a dropdown of valid codes */
        var _condCur = (l.condition_code || '').toString().toUpperCase();
        var _condOpts = ['', 'NE', 'NS', 'AR', 'OH', 'RP', 'SV', 'SC'].map(function(c){ var lbl = c || '&mdash;'; return '<option value="' + c + '"' + (_condCur === c ? ' selected' : '') + '>' + lbl + '</option>'; }).join('');
        html += '<div><div class="detail-label">Condition</div><select name="lines[' + idx + '][condition_code]" style="width:100%;background:#0e1828;border:1px solid #1e2d42;color:#eef1f5;padding:5px 8px;">' + _condOpts + '</select></div>';
