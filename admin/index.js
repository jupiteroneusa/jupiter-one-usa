...
export async function buildAdminRouter() {
  ...
  router.get('/orders', async (req, res) => {
    ...
    try {
      ...
      </tr>`).join('') || '<tr><td colspan="6" style="text-align:center;color:#7a8a9a;padding:24px;">No orders yet</td></tr>';
      res.send(page('Orders','orders',`
        <div class="page-title">Orders</div>
        <div class="page-sub">All customer orders</div>
        <div class="card">
          <table><thead><tr><th>Order #</th><th>Customer</th><th>Amount</th><th>Status</th><th>Date</th><th></th></tr></thead>
          <tbody>${rows}</tbody></table>
        </div>`));
    } catch(err) {
  ...
  router.get('/orders/:id', async (req, res) => {
    ...
    try {
      ...
      const lineRows = oLines.recordset.map(function(l) { return '<tr><td style="color:#7a8a9a;">'+l.line_number+'</td><td class="mono" style="color:#c8932a;">'+(l.nsn||l.part_number||'—')+'</td><td>'+(l.item_name||'—')+'</td><td>'+l.quantity_ordered+'</td><td style="color:#7a8a9a;">'+(l.condition_code||'—')+'</td><td style="font-weight:600;">$'+parseFloat(l.unit_price||0).toFixed(2)+'</td><td style="font-weight:600;">$'+parseFloat(l.line_total||0).toFixed(2)+'</td></tr>'; }).join('');
      const shipRows = ships.recordset.map(function(s) { return '<tr><td class="mono">'+(s.shipment_number||'')+'</td><td>'+(s.carrier||'—')+'</td><td>'+(s.tracking_number?'<a href="'+(s.tracking_url||'#')+'" target="_blank" style="color:#c8932a;">'+s.tracking_number+'</a>':'—')+'</td><td>'+statusBadge(s.status||'Pending')+'</td><td style="color:#7a8a9a;font-size:.78rem;">'+(s.ship_date?new Date(s.ship_date).toLocaleDateString():'—')+'</td><td style="color:#7a8a9a;font-size:.78rem;">'+(s.estimated_delivery?new Date(s.estimated_delivery).toLocaleDateString():'—')+'</td></tr>'; }).join('') || '<tr><td colspan="6" style="text-align:center;color:#7a8a9a;padding:12px;">No shipments yet</td></tr>';
      const logRows = sLog.recordset.map(function(l) { return '<tr><td style="color:#7a8a9a;font-size:.78rem;">'+new Date(l.created_at).toLocaleString()+'</td><td>'+statusBadge(l.new_status)+'</td><td style="color:#7a8a9a;">'+(l.note||'—')+'</td></tr>'; }).join('') || '<tr><td colspan="3" style="text-align:center;color:#7a8a9a;padding:12px;">No history</td></tr>';
      ...
      function tabLink(tab, label) { return '<a href="/admin/orders/'+o.id+'?tab='+tab+'" style="display:inline-block;padding:8px 18px;font-size:.82rem;font-weight:600;border-bottom:2px solid '+(activeTab===tab?'#c8932a':'transparent')+';color:'+(activeTab===tab?'#c8932a':'#7a8a9a')+';text-decoration:none;white-space:nowrap;">'+label+'</a>'; }
      let html = successMsg;
  ...
  router.post('/orders/:id/shipping', async (req, res) => {
    ...
    try {
      ...
      await pool.request().input('id',sql.BigInt,req.params.id).input('shipping',sql.Decimal(12,2),shippingCost).input('total',sql.Decimal(12,2),subtotal+shippingCost).input('addr1',sql.NVarChar(150),b.ship_to_address1||null).input('city',sql.NVarChar(100),b.ship_to_city||null).input('state',sql.NVarChar(50),b.ship_to_state||null).input('zip',sql.NVarChar(20),b.ship_to_zip||null).input('country',sql.NVarChar(50),b.ship_to_country||null).query('UPDATE orders SET shipping_cost=@shipping,total_amount=@total,ship_to_address1=@addr1,ship_to_city=@city,ship_to_state=@state,ship_to_zip=@zip,ship_to_country=@country,updated_at=GETDATE() WHERE id=@id');
  ...
  router.post('/orders/:id/tracking', async (req, res) => {
    ...
    try {
      ...
      await pool.request().input('orderId',sql.BigInt,req.params.id).input('shipNum',sql.NVarChar(20),shipNum).input('carrier',sql.NVarChar(100),b.carrier||null).input('tracking',sql.NVarChar(100),b.tracking_number||null).input('trackingUrl',sql.NVarChar(500),b.tracking_url||null).input('shipDate',sql.Date,b.ship_date||null).input('estDelivery',sql.Date,b.estimated_delivery||null).query("INSERT INTO shipments (order_id,shipment_number,carrier,tracking_number,tracking_url,ship_date,estimated_delivery,status) VALUES (@orderId,@shipNum,@carrier,@tracking,@trackingUrl,@shipDate,@estDelivery,'Shipped')");
