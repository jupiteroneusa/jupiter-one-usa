// patch-ccauth-rewrite.cjs
// Cleanly rewrite routes/ccAuth.js — previous version had escaped backticks
// from nested template literals that became literal \` in the file.

const fs = require('fs');
const { execSync } = require('child_process');

const TARGET = 'routes/ccAuth.js';
const BACKUP = TARGET + '.broken.bak';

if (fs.existsSync(TARGET)) {
  fs.writeFileSync(BACKUP, fs.readFileSync(TARGET));
}

// Use a single template literal stored as plain string with placeholders,
// not nested template literals. We build HTML with string concatenation
// or with .replace() on a plain string.
...
try {
  ...
  console.log('+ routes/ccAuth.js rewritten cleanly');
  console.log('+ Syntax check passed');
  console.log('+ File size: ' + Math.round(fs.statSync(TARGET).size / 1024) + ' KB');
  console.log('SUCCESS');
} catch (err) {
  console.error('! SYNTAX ERROR after rewrite:');
